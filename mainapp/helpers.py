...
def get_book_dict(bookid_list):
    ...
    # Now, for each book dictionary in rec_books_dict, add the 'price' field
    for book_dict in rec_books_dict:
        book_id = book_dict["book_id"]
        # Fetch price from the CSV based on book_id
        price = df_book[df_book["book_id"] == book_id]["Price"].values[0]
        book_dict["price"] = price

    return rec_books_dict
#END CHANGED----------------------------------END CHANGED--------------------------------END CHANGED
